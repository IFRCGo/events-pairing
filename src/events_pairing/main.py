...
def run_pipeline(events_df: pd.DataFrame, search_configs):
    ...
    for hazard, grp_df in events_df.groupby(by="hazard_type"):
        if not grp_df.shape[0]:
            logger.warning(f"Data not found for hazard {hazard.value}. Skipping...")
            continue
...
if __name__ == "__main__":
    ...
    final_processed_events_df = Utils.postprocess_event_df(events_df=processed_events_df)
    logger.info(f"Size of the processed events after postprocessing: {final_processed_events_df.shape}")

    grid_search_configs = {}
    for hazard_type in HazardType:
        logger.info(f"Processing {hazard_type.value}")
        part_config = GridSearch.run_grid_search(events_df=final_processed_events_df, hazard=hazard_type)
        if not part_config:
            logger.warning(f"Grid configs is not available for {hazard_type.value}")
            continue
        grid_search_configs.update({hazard_type.value: part_config})
    ...
    run_pipeline(events_df=final_processed_events_df, search_configs=validated_configs)
